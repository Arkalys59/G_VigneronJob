...
function Gestion()
    local open = false
    local mainMenu = RageUI.CreateMenu("Gestion Vigneron", "Menu")
...
local pos = {{x = G_VigneronJob.Pos.Gestion.x, y = G_VigneronJob.Pos.Gestion.y, z = G_VigneronJob.Pos.Gestion.z}}
    ...
    while true do
        ...
        for k in pairs(pos) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == "vigneron" and ESX.PlayerData.job.grade_name == "boss" then 
                ...
                if dist <= 2.0 then 
                    wait = 0
                    DrawMarker(G_VigneronJob.Marker.Type, G_VigneronJob.Pos.Gestion.x, G_VigneronJob.Pos.Gestion.y, G_VigneronJob.Pos.Gestion.z-1, 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, 0.5, 0.5, 0.5, G_VigneronJob.Marker.R, G_VigneronJob.Marker.V, G_VigneronJob.Marker.B, 255, false, false, p19, false)     
                    if dist <= 1.0 then 
                        wait = 0 
                        ESX.ShowHelpNotification("~INPUT_TALK~ pour ouvrir le ~p~Gestion")
